...
- (void)cancelRecording
{
   ...
   UnitySendMessage("IOSVoiceControl", "LogMessage", "recording has been stopped by user");
}
/*
...
-(void)returnResult
{
   ...
   UnitySendMessage("IOSVoiceControl", "onResults", [transcriptText UTF8String]);
}
...
-(void)startOnDevice
{
   ...
   recognitionTask =[speechRecognizer recognitionTaskWithRequest:recognitionRequest resultHandler:^(SFSpeechRecognitionResult * _Nullable result, NSError * _Nullable error)
   {
      if (result != nil) {
         ...
         cancelAfterInputTimer = [NSTimer scheduledTimerWithTimeInterval: 0.85 target: self selector:@selector(stopRecording:) userInfo:nil repeats:false];
